...
### 第6R【高並列の残課題: memcached churn / session往復 / GC】Go(main)。ローカルfail0(~280k)。
- ① memcached `MaxIdleConns=128`（DBプールと同型のchurn解消。session storeの接続churn対策・数行）。
- ② session値の in-process キャッシュ（sessionValsCache: cookie値→{uid,csrf,notice}）。getSessionUser/getCSRFToken/getFlash が memcached往復を回避。
  - 書込(login/register/logout/flash)は saveSess() で当該cookieキャッシュを無効化→次回読みで最新反映。flash(notice)は無い時memcached非接触、有る時のみ消費。
  - 検証: 匿名csrf空/ログインcsrf実トークン/**失敗投稿後の flash 表示OK**/コメント即時可視OK。
- ③ GOGC=200 を isu-go.service Environment に（systemd永続=再起動耐性）。2コア専有でGCがCPUを奪うのを抑制。
- ④(DBレスfeed)は feedクエリが既に index-only/in-memory で限界効用が小さく、整合リスク回避のため保留（必要なら後続）。
- 検証: 全ページ200・fail0・flash/コメント/csrf正常。→大会ベンチで検証(69万狙い)。
