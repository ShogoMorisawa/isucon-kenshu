...
# [17:1x] Go第2R（Go 479,617 採用 → トップ 693,053 / まだ約1.4倍 / 信号機IDLE時計測）

## 前提（適用確認済み）
接続まわりは対応済み（app.go: `cfg.Net="unix"`+`mysqld.sock` / `interpolateParams=true` / `SetMaxOpenConns(64)`/`SetMaxIdleConns(64)`/`SetConnMaxLifetime(0)`）。第1Rの3点は解決。**ここから先のDBレバーを探す。**

## 現状のDB負荷スナップショット（SHOW GLOBAL STATUS, 直近ベンチ累積）
- `Com_select=249,222` / `Com_insert=18,646` / `Com_update=7,293` → **読み:書き ≈ 10:1**。読みが依然主役。
- `Innodb_rows_read=5,991,653` → 約6M行読み。**断片キャッシュで描画は消えたが、DBへの読みクエリ(=毎GET/のフィードクエリ)はまだ生きている**。
- `Innodb_rows_updated=133,394`（うち大半はinitializeの一括UPDATE）。コメント毎の `comment_count+1` は1行更新。
- コメント分布: 1postあたり最大23・平均10。**書き込みは特定postに集中しにくい**（行ロック競合は低リスク）。

## ① 【最大の読みレバー】毎GET/のフィードクエリをプロセス内キャッシュで消し「GET / を完全DBレス」に
- (1)現象: getIndex(640) は断片全ヒットでも **毎回** `SELECT id,user_id,comment_count FROM posts USE INDEX(idx_feed) ORDER BY created_at DESC LIMIT 40` を実行（index-onlyだが1往復/最頻endpoint）。`Com_select=249k` の主成分はこれ。
- (2)着眼（Goの構造的優位）: **フィード上位40件の並びは新規投稿(POST /)でしか変わらない。コメントは順序を変えず comment_count だけ変える**。
  GoはPHPと違い**プロセス内メモリ**を持てる（既に fragmentCache/userCache/bannedSet が sync.Map で稼働中）。フィードのid一覧もメモリに置ける＝**memcache往復すら不要**（PHP第4R②の上位版）。
- (3)推奨アクション（実装係へ）:
  - プロセス内に「最近の投稿スライス」`[]struct{ID,UserID,CommentCount,CreatedAt}` を保持（初回はDBから上位N件ロード、以後メモリ更新）。
  - `POST /`(投稿): 新規postを先頭にprepend（comment_count=0）。
  - `POST /comment`: 当該postの **メモリ上 comment_count を++**（断片キーが変わり自動再描画されるのは現状通り）。
  - `getIndex`: このメモリスライスから上位40 → buildListPosts。**DBクエリゼロ**（断片ミス時のみ本体/コメントをDBへ）。
  - `getPosts`(max_created_at ページング)も同スライスから created_at で切り出せる範囲はメモリ供給。
  - 整合: del_flg は既存 bannedSet（in-process, admin/banned で無効化）で選別。initializeで全クリア（clearAppCaches済）。
- (4)根拠: 最頻GET/から最後の1往復を除去。`Com_select` の主成分が消え、DB読みCPU＋接続占有が大幅減。Goでmemcache不要のため往復ゼロ＝PHP第4R②より高速。**読み側の本丸。**

## ② comment_count が idx_feed のキー列 → コメント毎に index churn（書き込み増幅）
- (1)現象: `idx_feed=(created_at, comment_count, user_id)`。`POST /comment` の `UPDATE posts SET comment_count=comment_count+1`(1019) は **comment_count がインデックスのキー列(seq2)** なので、クラスタindex更新に加え**idx_feedの該当エントリを delete+reinsert**（値が変わると物理位置が動く）。コメントの度に二次index書き込みが発生。
- (2)原因仮説: 「フィードをindex-onlyで読む」ために comment_count をindexに入れた副作用。読みは速くなったが**最頻書き込み(POST /comment)に増幅コスト**を負わせている。高スコア帯は書き込みが律速化しやすく、ここが効いてくる。
- (3)推奨アクション（①とセットで最大効果）:
  - ①でフィードをメモリ供給するなら、**idx_feed はもう読みに不要**。→ `idx_feed` を **`(created_at, user_id)` か元の `idx_created_at(created_at)` に戻し comment_count をindexから外す**。これで UPDATE が二次index churnを起こさなくなる（クラスタindexの1行更新のみ）。
  - さらに踏み込むなら（①採用時）**comment_count の DB UPDATE 自体を撤廃**しメモリ値を真とする案も。ただし**ベンチはコメント数の整合を検証**するため、/@user 等DBから数える経路が残る限りDB値も正しく保つ必要あり→ 安全には「UPDATEは残すがindexからcomment_countを外す」が低リスク。
- (4)根拠: 最頻書き込みパスの二次index delete+insertを除去。①の読み最適化と両立し、書き込み律速を緩和。**①と合わせて評価すべき。**

## ③ POST /comment の2往復 → ①採用なら1往復化の余地
- (1)現象: postComment は `INSERT comments`(1012) と `UPDATE posts ...`(1019) の **2回の ExecContext = 2往復・2接続取得**。
- (2)推奨アクション: ①でメモリ comment_count を持てば、DBの `UPDATE posts` を**非同期化（チャネルでまとめて遅延反映）or バッチ化**して critical path から外せる。最小工数なら現状維持でも可（②のindex churn除去が効けばUPDATE自体は軽くなる）。
- (3)根拠: 書き込みパスのDB往復・接続占有を半減。高並列の書き込みバーストで効く。②の次。

## ④ /@user(getAccountName) の往復過多（中）
- (1)現象: `SELECT * users WHERE account_name`(674) + `posts WHERE user_id`(687) + **同じ `SELECT id FROM posts WHERE user_id`(707) を重複** + `COUNT comments WHERE user_id`(700) + `COUNT comments WHERE post_id IN(...)`(728)。**1ページで最大5往復**。
- (2)推奨アクション:
  - 707は687の結果からid流用で**削除**（1往復減）。
  - commentedCount(自投稿への被コメント総数) は `SELECT SUM(comment_count) FROM posts WHERE user_id=?`(idx_user_created range)で**comments を引かず算出**→728を消す。
  - postCount/commentCount(本人コメント数)は users非正規化列で持てば700も消せる（POST //comment で増分）。
- (3)根拠: /@user を5→2往復へ。GET/ほどではないが頻度ぶん効く。

## ⑤ ユーザ(account_name)ログインのDB読み（小〜中）
- (1)現象: `tryLogin`(104) は毎ログインで `SELECT * users WHERE account_name=? AND del_flg=0`（unique index）。passhash検証に必要でキャッシュしにくい。
- (2)推奨アクション: 同一userの反復ログインがあるなら、**account_name→User を sync.Map にキャッシュ**（passhash含む、del_flg/ban時に無効化）。ベンチが同じアカウントで何度もログインするなら往復削減。効果はベンチ挙動依存＝A/Bで判断。
- (3)根拠: login系シナリオの往復削減。確証は低いので①〜④の後。

## ⑥ 接続プール64の最適性（A/B枠）
- (1)現状: `SetMaxOpenConns(64)/SetMaxIdleConns(64)`。①採用で**読みクエリが激減**すると、プールの主用途は書き込み＋ミス＋loginに移る。
- (2)見立て: ①後はDB同時実行数が下がるので**64は過剰寄りになる可能性**。逆に書き込みバースト時は枯渇させたくない。**ベンチ中の mysqld CPU と `Threads_running` を見て 32/48/64/96 をA/B**。mysqldがCPU非飽和なら増やしても無益、飽和なら絞る方が良い場合も。
- (3)根拠: プールは「足りないと枯渇・多すぎるとコンテキスト/ロック競合」。①でDB負荷プロファイルが変わるので**①適用後に再チューニング**するのが筋。単独での伸びは不確実。

## 69万点チームがDB側でやっている可能性（推測）
1. **GET / を完全にメモリ供給（DBレス）**＝①。Goの単一プロセス強みでフィード・カウント・ユーザを全てin-process化し、ホット読みのDB往復をゼロに。
2. **書き込みの軽量化**: comment_count を index から外す/メモリ化（②）、UPDATE非同期・バッチ（③）。最頻書き込みのDBコストを最小化。
3. **users マスタの全in-processキャッシュ**（数千行）＋account_name索引もメモリ。loginを除き users SELECT を全廃。
4. **DBを別サーバへ分離**しアプリ機CPUを独占（CPUバウンドなら最大の構造レバー。要サーバ台数確認）。
5. 場合により **INSERT のバッチ/まとめcommit**、`innodb_flush_log_at_trx_commit=2`（既設）等で書き込みfsyncを軽量化。

## Go第2Rまとめ（実装係向け・効果順）
1. **①フィードをプロセス内キャッシュ化し GET / を完全DBレスに**（Goの強み・memcache不要・最頻読みの最後の1往復を除去）★最優先
2. **②idx_feed から comment_count を外し、コメント毎の二次index churnを止める**（①とセット。最頻書き込みの増幅除去）★①と同時評価
3. **③POST /comment の UPDATE を非同期/バッチ化で1往復化**（書き込みパス短縮）
4. **④/@user の往復削減**（707重複削除＋commentedCountを`SUM(comment_count)`化＋users非正規化）
5. ⑤loginのuserキャッシュ（A/B）・⑥①適用後にプール値32〜96をA/B再チューニング
- 🔑 結論: **読みは①でDBから消し、書きは②③で増幅と往復を削る**のが479k→693kのDB側の道。Goは単一プロセスのin-processキャッシュでmemcache往復すら省ける構造優位があり、ここを使い切るのが本丸。純index/my.cnfの伸びは無い。
